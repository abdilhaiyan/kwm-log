...
**Domain:** No-build, single-file CDN React app — safe decomposition into multiple files while keeping the serve-a-folder deployment model
...
**Headline:** Keep every CDN script exactly where it is. The decomposition mechanism is **multiple external `<script type="text/babel" src="js/*.jsx">` files** processed by the Babel standalone already in the page. This is the only clean way to get real multi-file JSX without a build step over `python -m http.server` (verified: native ES modules would hit a MIME-type wall for `.jsx` files, and no CDN JSX loader supports external files). Do NOT introduce Vite/ESM/import maps for this milestone — payoffs don't justify the risk or the toolchain.
...
```
index.html            — unchanged <head> CDN scripts + Tailwind config + <style>; body carries the
                        decomposed <script type="text/babel" src="…"> tags in dependency order
js/
  constants.js        — STATUS_COLORS, INPUT_CLS, SELECT_CLS, DEFAULT_PASSCODE, appId logic (window.__app_id)
  firebase.js         — firebaseConfig, initializeApp/auth/firestore inside the existing try/catch
  utils.js            — isOverdue, getDaysInfo, getRequestDuration, getEquipmentLabel, getEquipmentIcon,
                        copyDetails (clipboard 3-tier fallback), exportCSV
  data.js             — passcode fetch effect + live requests onSnapshot + lucide.createIcons() re-run
  ui/icons.jsx        — Icon component (lucide.icons[name] wrapper)
  views/form.jsx      — renderFormFields + submitRequest handler wiring
  views/manager.jsx   — filter bar, status chips, request cards (incl. per-card approve/reject/passed/return actions)
  views/modals.jsx    — passcode modal, return-condition modal, detail modal
  App.jsx             — App component: all useState hooks + derived memos + composition of the above
  main.js             — ReactDOM.createRoot(...).render(<App />)
```

Script order in `index.html` = topological dependency order. The current inline block's 19 `useState` hooks stay in `App.jsx` (behavior-preserving first; hook extraction into custom hooks is a *later* refinement, not this milestone).
...
There is no installation — that is the constraint and the deliverable. No `npm install`, no `package.json`, no `node_modules`, no build output.
...
Pin the CDN URLs (recommended exact URLs, all HEAD-verified live 2026-09-11):
...
<!-- Tailwind and Firebase URLs unchanged -->
```
...
**If the milestone is "decompose with zero behavior change" (current):**
...
**If the app later needs npm packages / TS / HMR (signal: "I want to import lodash"):**
...
**If the app later keeps no-build but needs real ESM (signal: hard encapsulation requirements):**
...
---
*Stack research for: KWM Logistics Equipment Log — no-build decomposition of a monolithic CDN React app*
*Researched: 2026-09-11*